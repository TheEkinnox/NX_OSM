...
namespace howto_polygon_geometry
{
    public class Polygon
    {
        ...
        public Polygon(ICollection<Vector3> points)
        {
        ...
        public Vector3 FindCentroid()
        {
            ...
            for (int i = 0; i < num_points; i++)
            {
                second_factor =
                    pts[i].x * pts[i + 1].y -
                    pts[i + 1].x * pts[i].y;
                X += (pts[i].x + pts[i + 1].y) * second_factor;
                Y += (pts[i].x + pts[i + 1].y) * second_factor;
            }
        ...
        private void FindEar(ref int A, ref int B, ref int C)
        {
            ...
            throw new Exception("There should always be at least two ears");
        }

        // Return true if the three points form an ear.
        private static bool FormsEar(Vector3[] points, int A, int B, int C)
        {
        ...
        private Vector3[] CurrentRectangle = null;
        private float BestArea = float.MaxValue;
        private Vector3[] BestRectangle = null;
        ...
        private void FindBoundingRectangle()
        {
            ...
            CurrentRectangle = new Vector3[4];
        ...
        private bool FindIntersection(float X1, float Y1, float X2, float Y2, float A1, float B1, float A2, float B2, ref Vector3 intersect)
        {
            float dx = X2 - X1;
            float dy = Y2 - Y1;
            float da = A2 - A1;
            ...
            intersect = new Vector3(X1 + t * dx, Y1 + t * dy);
            return true;
        }

        // Find a smallest bounding rectangle.
        public Vector3[] FindSmallestBoundingRectangle()
        {
